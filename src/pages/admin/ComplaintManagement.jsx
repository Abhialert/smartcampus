import { useState } from 'react';
import { useApp } from '../../contexts/AppContext';
import { complaintCategories } from '../../data/campusData';
import {
  ClipboardList,
  Clock,
  CheckCircle2,
  Loader2,
  AlertTriangle,
  Filter,
  MessageSquare,
  X,
} from 'lucide-react';

export default function ComplaintManagement() {
  const { complaints, updateComplaintStatus } = useApp();
  const [filter, setFilter] = useState('all');
  const [selectedComplaint, setSelectedComplaint] = useState(null);
  const [remarks, setRemarks] = useState('');
  const [newStatus, setNewStatus] = useState('');
  const [showSuccess, setShowSuccess] = useState(false);

  const filteredComplaints = filter === 'all'
    ? complaints
    : complaints.filter(c => c.status === filter);

  const statusCounts = {
    all: complaints.length,
    pending: complaints.filter(c => c.status === 'pending').length,
    'in-progress': complaints.filter(c => c.status === 'in-progress').length,
    resolved: complaints.filter(c => c.status === 'resolved').length,
  };

  const getCategoryInfo = (catId) => {
    return complaintCategories.find(c => c.id === catId) || { icon: '📝', label: catId, color: '#8b5cf6' };
  };

  const handleStatusUpdate = () => {
    if (selectedComplaint && newStatus) {
      updateComplaintStatus(selectedComplaint.id, newStatus, remarks);
      setSelectedComplaint(null);
      setRemarks('');
      setNewStatus('');
      setShowSuccess(true);
      setTimeout(() => setShowSuccess(false), 3000);
    }
  };

  const filterButtons = [
    { key: 'all', label: 'All', icon: ClipboardList },
    { key: 'pending', label: 'Pending', icon: Clock },
    { key: 'in-progress', label: 'In Progress', icon: Loader2 },
    { key: 'resolved', label: 'Resolved', icon: CheckCircle2 },
  ];

  return (
    <div className="space-y-6 max-w-6xl">
      {/* Header */}
      <div className="animate-fade-in">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <ClipboardList className="w-6 h-6 text-accent-light" />
          Complaint Management
        </h1>
        <p className="text-text-muted text-sm mt-1">Review and manage student complaints</p>
      </div>
      {showSuccess && (
        <div className="flex items-center gap-2 px-4 py-2 bg-success/15 border border-success/30 rounded-xl text-success animate-fade-in w-fit">
          <CheckCircle2 className="w-4 h-4" />
          <span className="text-xs font-bold">Update Successful!</span>
        </div>
      )}

      {/* Status Filters */}
      <div className="flex gap-2 flex-wrap animate-fade-in">
        {filterButtons.map((btn) => (
          <button
            key={btn.key}
            onClick={() => setFilter(btn.key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all active:scale-95 ${
              filter === btn.key
                ? 'bg-accent/15 text-accent-light border border-accent/30'
                : 'glass hover:bg-surface-light/50 text-text-secondary border border-transparent'
            }`}
          >
            <btn.icon className={`w-4 h-4 ${btn.key === 'in-progress' && filter === btn.key ? 'animate-spin' : ''}`} />
            {btn.label}
            <span className="text-[10px] bg-surface-light/50 px-1.5 py-0.5 rounded-full">
              {statusCounts[btn.key]}
            </span>
          </button>
        ))}
      </div>

      {/* Complaints List */}
      <div className="glass rounded-xl overflow-hidden animate-fade-in">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-glass-border text-xs text-text-muted">
                <th className="text-left p-4 font-medium">Complaint</th>
                <th className="text-left p-4 font-medium">Category</th>
                <th className="text-left p-4 font-medium">By</th>
                <th className="text-left p-4 font-medium">Date</th>
                <th className="text-left p-4 font-medium">Status</th>
                <th className="text-left p-4 font-medium">Action</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-glass-border">
              {filteredComplaints.map((c) => {
                const catInfo = getCategoryInfo(c.category);
                return (
                  <tr key={c.id} className="hover:bg-surface-light/20 transition-colors">
                    <td className="p-4">
                      <p className="text-sm font-medium">{c.title}</p>
                      <p className="text-[11px] text-text-muted mt-0.5 line-clamp-1">{c.description}</p>
                      {c.location && (
                        <p className="text-[10px] text-text-muted mt-0.5">📍 {c.location}</p>
                      )}
                    </td>
                    <td className="p-4">
                      <span className="text-xs px-2 py-1 rounded-full" style={{ backgroundColor: catInfo.color + '20', color: catInfo.color }}>
                        {catInfo.icon} {catInfo.label}
                      </span>
                    </td>
                    <td className="p-4">
                      <p className="text-sm">{c.submittedName}</p>
                      {c.category === 'ragging' && c.submittedBy === 'anonymous' && (
                        <span className="text-[9px] bg-danger/15 text-danger px-1.5 py-0.5 rounded-full">Anonymous</span>
                      )}
                    </td>
                    <td className="p-4 text-sm text-text-muted">{c.date}</td>
                    <td className="p-4">
                      <span className={`text-[10px] px-2.5 py-1 rounded-full font-medium flex items-center gap-1 w-fit ${
                        c.status === 'pending' ? 'bg-warning/15 text-warning' :
                        c.status === 'in-progress' ? 'bg-info/15 text-info' :
                        'bg-success/15 text-success'
                      }`}>
                        {c.status === 'pending' && <Clock className="w-3 h-3" />}
                        {c.status === 'in-progress' && <Loader2 className="w-3 h-3 animate-spin" />}
                        {c.status === 'resolved' && <CheckCircle2 className="w-3 h-3" />}
                        {c.status}
                      </span>
                    </td>
                    <td className="p-4">
                      <button
                        onClick={() => { setSelectedComplaint(c); setNewStatus(c.status); setRemarks(c.remarks || ''); }}
                        className="text-xs text-accent-light hover:text-accent transition-colors px-3 py-1.5 rounded-lg hover:bg-accent/10 active:scale-95"
                      >
                        Manage
                      </button>
                    </td>
                  </tr>
                );
              })}
            </tbody>
          </table>
        </div>
        {filteredComplaints.length === 0 && (
          <div className="p-12 text-center">
            <CheckCircle2 className="w-12 h-12 text-success/30 mx-auto mb-3" />
            <p className="text-text-muted text-sm">No complaints in this category</p>
          </div>
        )}
      </div>

      {/* Manage Modal */}
      {selectedComplaint && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="glass-strong rounded-2xl w-full max-w-lg">
            <div className="p-6 border-b border-glass-border flex items-center justify-between">
              <h2 className="text-lg font-semibold">Manage Complaint</h2>
              <button onClick={() => setSelectedComplaint(null)} className="p-1 hover:bg-surface-light rounded-lg transition-colors">
                <X className="w-5 h-5 text-text-muted" />
              </button>
            </div>

            <div className="p-6 space-y-5">
              {/* Complaint Info */}
              <div className="glass rounded-xl p-4">
                <h3 className="font-medium text-sm">{selectedComplaint.title}</h3>
                <p className="text-xs text-text-muted mt-1">{selectedComplaint.description}</p>
                <div className="flex items-center gap-3 mt-2 text-[10px] text-text-muted">
                  <span>{selectedComplaint.date}</span>
                  <span>by {selectedComplaint.submittedName}</span>
                  <span>📍 {selectedComplaint.location}</span>
                </div>
              </div>

              {/* Status Update */}
              <div>
                <label className="text-xs font-medium text-text-secondary mb-2 block">Update Status</label>
                <div className="grid grid-cols-3 gap-2">
                  {['pending', 'in-progress', 'resolved'].map((status) => (
                    <button
                      key={status}
                      onClick={() => setNewStatus(status)}
                      className={`p-2.5 rounded-xl border text-xs font-medium capitalize transition-all ${
                        newStatus === status
                          ? status === 'pending' ? 'border-warning/50 bg-warning/10 text-warning' :
                            status === 'in-progress' ? 'border-info/50 bg-info/10 text-info' :
                            'border-success/50 bg-success/10 text-success'
                          : 'border-glass-border hover:bg-surface-light/30 text-text-secondary'
                      }`}
                    >
                      {status}
                    </button>
                  ))}
                </div>
              </div>

              {/* Remarks */}
              <div>
                <label className="text-xs font-medium text-text-secondary mb-1.5 block">Admin Remarks</label>
                <textarea
                  value={remarks}
                  onChange={(e) => setRemarks(e.target.value)}
                  placeholder="Add a remark or update for the student..."
                  rows={3}
                  className="w-full bg-surface/50 text-sm text-text-primary px-4 py-3 rounded-xl border border-glass-border focus:outline-none focus:border-accent/50 transition-all placeholder:text-text-muted resize-none"
                />
              </div>
            </div>

            <div className="p-6 border-t border-glass-border flex gap-3">
              <button
                onClick={() => setSelectedComplaint(null)}
                className="flex-1 py-2.5 rounded-xl border border-glass-border hover:bg-surface-light/50 text-sm transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleStatusUpdate}
                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-accent to-blue-500 text-white text-sm font-medium transition-all active:scale-95"
              >
                Update Status
              </button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}
